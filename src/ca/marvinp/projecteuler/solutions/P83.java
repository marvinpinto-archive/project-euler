...
public class P83 {
  ...
  public P83() {
    ...
    answer = dijkstraSum(0,0).toString();
  ...
  private BigInteger dijkstraSum(int x, int y) {
    ...
    // printMatrices();
    return dMatrix[dimension-1][dimension-1];
  }
  ...
  private void relaxNeighbours(Node u) {
    ...
    // Process the node to the left of us if there is a node to the left of us
    // that hasn't been visited
    if ( (y > 0) && !visitedArr[x][y-1]) {
      int linkCost = matrix[x][y-1];
      BigInteger weightedCost = myD.add(BigInteger.valueOf(linkCost));
      if ( weightedCost.compareTo(dMatrix[x][y-1]) < 0 ) {
        processNode(x, y-1, weightedCost);
  ...
  private int[][] loadMatrix() {
    ...
    try {
      InputStream is = P83.class.getClassLoader()
        .getResourceAsStream(Constants.INPUT_PATH + "p83.txt");
  ...
  public String getQuestion() {
    StringBuilder sb = new StringBuilder();
    sb.append("URL: https://projecteuler.net/problem=83" + Constants.NEWLINE);
